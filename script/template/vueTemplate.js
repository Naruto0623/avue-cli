  ...
  <div class="${ compoenntName }">
    <Modal v-model="isOpen" width="50%">
        ...
        <Form :model="addItem" :label-width="120" :rules="rules" ref="formItem">
          <Col span="11">
            <FormItem label="名称" prop="name">
              <Input clearable size="small" v-model.trim="addItem.name" placeholder="请输入用户名"></Input>
            </FormItem>
          </Col>
          <Col span="11" style="height: 42px;">
            <FormItem label="状态" prop="isActive">
              <Select size="small" v-model="addItem.isActive" placeholder="请选择状态">
      ...
      <div slot="footer">
        ...
        <Button type="info" size="small" :loading="isLoading" @click="infoSave">保存</Button>
      </div>
    </Modal>
  </div>
</template>
